...
// Flow Canvas (lazy loaded - Phase 0)
const FlowCanvasPage = lazy(() => import("./src/flowCanvas/FlowCanvasPage"));
const ImageLibraryPage = lazy(() => import("./src/flowCanvas/pages/ImageLibraryPage"));
const ImageEditMappingPage = lazy(() => import("./src/flowCanvas/pages/ImageEditMappingPage"));
...
const App: React.FC = () => {
  ...
  const isAssetLibraryRoute =
    typeof window !== "undefined" &&
    currentPath.startsWith("/assets");
  const isModelMappingRoute =
    typeof window !== "undefined" &&
    currentPath.startsWith("/model-mapping");
  const isFlowCreateRoute =
    typeof window !== "undefined" &&
    currentPath.startsWith("/create/flow");
  ...
  if (isAssetLibraryRoute) {
    return (
      <Suspense fallback={<div style={{ position: 'fixed', inset: 0, background: '#09090f', display: 'grid', placeItems: 'center', color: '#e2e8f0' }}>Loading Asset Library...</div>}>
        <ImageLibraryPage />
      </Suspense>
    );
  }

  if (isModelMappingRoute) {
    return (
      <Suspense fallback={<div style={{ position: 'fixed', inset: 0, background: '#09090f', display: 'grid', placeItems: 'center', color: '#e2e8f0' }}>Loading Model Mapping...</div>}>
        <ImageEditMappingPage />
      </Suspense>
    );
  }

  if (isFlowCreateRoute) {
    return (
      <Suspense
        fallback={
          <div style={{ position: 'fixed', inset: 0, background: '#0a0a12', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ color: '#e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
